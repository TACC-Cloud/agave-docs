...
Tapis (Agave) is reactive in its interactions with a remote file system. That is an advantage in that it does not place additional load on the system, however it creates challenges in situations where Agave is just one of many ways users are accessing the system. The file indexing service provides a bridge by which users can "refresh" Agave's knowledge of the data on a system.
